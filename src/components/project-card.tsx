...
<<<<<<< HEAD
  ...
  // Efek untuk mengambil user ID langsung dari Supabase jika halaman luar lupa mengirimkannya
  useEffect(() => {
    if (!propCurrentUserId) {
      ...
      setActiveUserId(propCurrentUserId);
    }
  }, [propCurrentUserId]);

  // Efek untuk memuat gambar dari Supabase Storage
  useEffect(() => {
    let active = true;
...
<<<<<<< HEAD
    ...
    try {
      // 1. Hapus data dari tabel projects
      ...
      // 2. Hapus file gambar dari bucket storage
      await supabase.storage.from("project-images").remove([project.image_url]);

      toast.success("Project dihapus.");
      
      // 3. Update tampilan (bisa lewat fungsi onDeleted atau memuat ulang halaman)
      if (onDeleted) {
